...
function createRandomString() {
    ...
    do {
        ...
        const randomBytes = crypto.randomBytes(length); // Génère un tableau d'octets aléatoires

        randomBytes.forEach((byte) => {
            result += chars[byte % chars.length]; // Construit la chaîne aléatoire
        });
    } while (ids.includes(result)); // Vérifie si l'ID généré existe déjà
